'use client';

import { Button, TextInput } from 'flowbite-react';
import Link from 'next/link';
import Image from 'next/image';
import {
    MapPinIcon,
    PhoneIcon,
    EnvelopeIcon
} from '@heroicons/react/24/outline';

export default function Footer() {
    const quickLinks = [
        { href: '/about-us', label: 'About Us' },
        { href: '/contact', label: 'Contact Us' },
        { href: '/services', label: 'Our Services' },
        { href: '/terms', label: 'Terms & Condition' },
        { href: '/support', label: 'Support' },
    ];

    const popularLinks = [
        { href: '/labs', label: 'Labs' },
        { href: '/projects', label: 'Projects' },
        { href: '/opportunities', label: 'Opportunities' },
        { href: '/publications', label: 'Publications' },
        { href: '/activities', label: 'Activities' },
    ];

    const socialLinks = [
        { href: '#', icon: 'twitter', label: 'Twitter' },
        { href: '#', icon: 'facebook', label: 'Facebook' },
        { href: '#', icon: 'linkedin', label: 'LinkedIn' },
        { href: '#', icon: 'instagram', label: 'Instagram' },
    ];

    return (
        <>
            {/* Footer Start */}
            <footer
                className="text-gray-400 py-12 relative min-h-[400px] bg-gray-900"
                style={{
                    backgroundImage: 'url(/footer.png)',
                    backgroundSize: 'cover',
                    backgroundPosition: 'center',
                    backgroundRepeat: 'no-repeat',
                    backgroundAttachment: 'local',
                }}
            >
                {/* Overlay for better text readability - reduced opacity to show image */}
                <div className="absolute inset-0 bg-gray-900/30 dark:bg-gray-900/40"></div>
                <div className="container mx-auto px-4 relative z-10">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                        {/* Left Column */}
                        <div className="lg:pr-8">
                            <Link href="/" className="inline-block mb-4">
                                <div className="flex items-center space-x-3">
                                    <Image
                                        src="/flowbite.svg"
                                        alt="Innovation Labs Logo"
                                        width={40}
                                        height={40}
                                        className="h-10 w-10"
                                    />
                                    <h1 className="text-2xl font-bold">
                                        <span className="text-blue-500">Innovation</span>
                                        <span className="text-white"> Labs</span>
                                    </h1>
                                </div>
                            </Link>
                            <p className="text-base mb-6 text-gray-300">
                                Leading research and development in cutting-edge technologies.
                                We foster innovation through collaboration, research, and practical applications.
                            </p>
                            <div className="space-y-3 mb-6">
                                <div className="flex items-center space-x-3">
                                    <MapPinIcon className="h-5 w-5 text-blue-500 shrink-0" />
                                    <p className="text-sm">D4 Street, Long Thanh My Ward, 9 District, Ho Chi Minh City, Vietnam</p>
                                </div>
                                <div className="flex items-center space-x-3">
                                    <PhoneIcon className="h-5 w-5 text-blue-500 shrink-0" />
                                    <p className="text-sm">+012 345 67890</p>
                                </div>
                                <div className="flex items-center space-x-3">
                                    <EnvelopeIcon className="h-5 w-5 text-blue-500 shrink-0" />
                                    <p className="text-sm">info@innovationlabs.com</p>
                                </div>
                            </div>
                            <div className="flex space-x-2 mt-6">
                                {socialLinks.map((social) => (
                                    <Button
                                        key={social.icon}
                                        as={Link}
                                        href={social.href}
                                        color="primary"
                                        size="lg"
                                        className="!p-3 !rounded-lg"
                                        aria-label={social.label}
                                    >
                                        <svg
                                            className="w-5 h-5"
                                            fill="currentColor"
                                            viewBox="0 0 24 24"
                                            aria-hidden="true"
                                        >
                                            {social.icon === 'twitter' && (
                                                <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
                                            )}
                                            {social.icon === 'facebook' && (
                                                <path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" />
                                            )}
                                            {social.icon === 'linkedin' && (
                                                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                                            )}
                                            {social.icon === 'instagram' && (
                                                <path d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z" />
                                            )}
                                        </svg>
                                    </Button>
                                ))}
                            </div>
                        </div>

                        {/* Right Column */}
                        <div className="lg:pl-8">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-8">
                                {/* Quick Links */}
                                <div>
                                    <h4 className="text-white text-lg font-semibold mb-4">Quick Links</h4>
                                    <div className="flex flex-col space-y-2">
                                        {quickLinks.map((link) => (
                                            <Link
                                                key={link.href}
                                                href={link.href}
                                                className="flex items-center gap-2 text-sm text-gray-400 hover:text-blue-500 transition-colors duration-200"
                                            >
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    fill="none"
                                                    viewBox="0 0 24 24"
                                                    strokeWidth={1.5}
                                                    stroke="currentColor"
                                                    className="size-4"
                                                >
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="m5.25 4.5 7.5 7.5-7.5 7.5m6-15 7.5 7.5-7.5 7.5" />
                                                </svg>
                                                {link.label}
                                            </Link>
                                        ))}
                                    </div>
                                </div>

                                {/* Popular Links */}
                                <div>
                                    <h4 className="text-white text-lg font-semibold mb-4">Popular Links</h4>
                                    <div className="flex flex-col space-y-2">
                                        {popularLinks.map((link) => (
                                            <Link
                                                key={link.href}
                                                href={link.href}
                                                className="flex items-center gap-2 text-sm text-gray-400 hover:text-blue-500 transition-colors duration-200"
                                            >
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    fill="none"
                                                    viewBox="0 0 24 24"
                                                    strokeWidth={1.5}
                                                    stroke="currentColor"
                                                    className="size-4"
                                                >
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="m5.25 4.5 7.5 7.5-7.5 7.5m6-15 7.5 7.5-7.5 7.5" />
                                                </svg>
                                                {link.label}
                                            </Link>
                                        ))}
                                    </div>
                                </div>
                            </div>

                            {/* Newsletter */}
                            <div>
                                <h4 className="text-white text-lg font-semibold mb-4">Newsletter</h4>
                                <p className="text-sm text-gray-400 mb-4">
                                    Subscribe to our newsletter to get the latest updates.
                                </p>
                                <form className="flex gap-2">
                                    <TextInput
                                        type="email"
                                        placeholder="Your Email Address"
                                        className="flex-1 bg-gray-800 border-gray-700 text-white placeholder-gray-500 focus:border-blue-500 focus:ring-blue-500"
                                        required
                                    />
                                    <Button type="submit" color="primary" className="px-6">
                                        Sign Up
                                    </Button>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>
            </footer>

            {/* Copyright Start */}
            <div className="bg-gray-950 text-gray-400 py-4 border-t border-gray-800">
                <div className="container mx-auto px-4">
                    <div className="flex flex-col md:flex-row justify-between items-center space-y-2 md:space-y-0">
                        <p className="text-sm text-center md:text-left">
                            &copy; <Link href="/" className="hover:text-blue-500 transition-colors">Innovation Labs</Link>. All Rights Reserved.
                        </p>
                        <p className="text-sm text-center md:text-right">
                            Designed by ABAD Team
                        </p>
                    </div>
                </div>
            </div>
            {/* Copyright End */}
        </>
    );
}

